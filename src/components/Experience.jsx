    ...
    <section id="experience" className="py-24 bg-[#f5f6fa] dark:bg-slate-950 relative transition-colors duration-300 section-glow overflow-x-hidden">
      <div ref={ref} className={`max-w-4xl mx-auto px-6 ${isRevealed ? '' : 'opacity-0'}`}>
        ...
        <div className="relative">
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-indigo-300 dark:from-indigo-500/40 to-purple-300 dark:to-purple-500/40 transform md:-translate-x-px"></div>

          {experience.map((exp, index) => (
            <div key={index} className={`relative pl-10 mb-10 last:mb-0 ${isRevealed ? 'reveal revealed' : 'reveal'}`} style={{ transitionDelay: `${index * 200}ms` }}>
              <div className="absolute left-2.5 w-3 h-3 bg-indigo-600 dark:bg-indigo-400 rounded-full border-2 border-white dark:border-slate-950 shadow z-10"></div>
